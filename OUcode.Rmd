...
```{r}
## Use PF at l=10 with 200 replications, to obtain the true value.
## Need to import package e1071 in R

ltemp = 10
# Number of particles N
N = 100 * 2^ltemp
# Time horizon, number of time units
T = 100
# Replications number of PF algorithm to get it's approximate variance, bias and MSE
R = 200
# l here is the discretization level
l = ltemp
# Initialization
...
W = W+1
for (p in 1:T) {
  ## Below is Euler update taking place in time p-1 to p, Xun[p,] represent singals at time p-1 , firstly we use Euler discretization to update Xun[p+1,] from Xun[p,], the goal is to get Xun1[p+1,], which we wiil obtain by perform resample step on Xun[p+1,]
  for(i in 1:N){
  ## particle update using Euler
  xdis = vector(mode = "numeric", length =(2^l+1))
  xdis[1] = Xun1[p,i]
  for (j in 1:(2^l)) {
      dw = rnorm(1, mean=0, sd = sqrt(0.5^l))
      xdis[j+1] = (-xdis[j])*0.5^l + 0.5*dw + xdis[j]
  }
  Xun[p+1,i]=xdis[(2^l+1)]
  ## G weights Function update
  ...
  ess[p]=(sum(w^2))^(-1)
  ## If ESS is smaller or equal than the threshold, we perform resample
  if(ess[p]<=(N/2)){
  ...
  ## If ESS is larger than the threshold, we don't resample
  if(ess[p]>(N/2)){
    Xun1[p+1,]=Xun[p+1,]
  }
...
# OU PF
```{r}
## Gradient Estimator, dYt = Xt*dt + dBt, dXt = (-Xt)*dt + 0.5*dWt (Using change of measure)
date()
...
# rate for PF here is -2.724165
```


# OU MLPF
```{r}
# Gradient Estimator, dYt = Xt*dt+dBt, dXt = (-Xt)*dt + 0.5*dWt 
...
# Replications number of PF algorithm to get it's approximate variance, bias and MSE
R=200
# This vector is used to store the estimate values over 200 replications for MLPF maximal resampling algorithm
...
for (p in 1:T) {
  ...
  ## Finer path Euler update
  fdis = vector(mode = "numeric", length =(2^l+1))
  fdis[1] = Xun1[p,i]
  for (j in 1:(2^l)) {
      dw[j] = rnorm(1, mean=0, sd = sqrt(0.5^l))
      fdis[j+1] = (-fdis[j])*0.5^l + 0.5*dw[j] + fdis[j]
  }
  Xun[p+1,i]=fdis[(2^l+1)]
  ##Coarser path Euler update
  cdis = vector(mode = "numeric", length =(2^(l-1)+1))
  cdis[1] = Xunc1[p,i]
  for(j in 1:(2^(l-1))){
    cdis[j+1] = (-cdis[j])*0.5^(l-1) + 0.5*(dw[2*j]+dw[2*j-1]) + cdis[j]
  }
  ...
  ## If Effective sample size larger than the threshold, we don't resample
  if(ESS1>(N/2)){
    Xun1[p+1,]=Xun[p+1,]
    Xunc1[p+1,]=Xunc[p+1,]
  }
}
## Estimates for Coupled Particle Filter, this is the summand term in the telescopic summation at l
for (i in 1:N) {
...
for (p in 1:T) {
  ## Loop over particles 
  for(i in 1:N){
  ## Particle update for one time unit using Euler discretization
  ## Finer path particles
  fdis = vector(mode = "numeric", length =2)
  fdis[1] = Xun1[p,i]
  dw = rnorm(1, mean=0, sd = 1)
  fdis[2] = (-fdis[1]) + 0.5*dw + fdis[1]
  Xun[p+1,i]=fdis[2]
...
## Compute estimate value for l=0
for (i in 1:N) {
  Bb[i] = wf[i]*Xun[T+1,i]
}
Aa[1] = sum(Bb)
## Cauculate the final estimator value for MLPF
estMLPF[h] = sum(Aa)
}
...
date()
## nothing wrong with this, apparently
```
...
for (p in 1:T) {
  ...
  ## Effective Sample Size
  ESS1 = (sum(wf^2))^(-1)
  ## If the ESS is smaller than the threshold, we resample
  if(ESS1<=(N/2)){
    If = vector(mode = "numeric",length = N)
    Ic = vector(mode = "numeric",length = N)
  ...
  ## If the ESS is larger than the threshold, we don't resample
  if(ESS1>(N/2)){
    Xun1[p+1,]=Xun[p+1,]
    Xunc1[p+1,]=Xunc[p+1,]
  }
