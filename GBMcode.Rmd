...
for (p in 1:T) {
  ...
  for (j in 1:(2^l)) {
      ...
      xdis[j+1] = 0.02*xdis[j]*0.5^l + 0.2*xdis[j]*dw[j] + xdis[j]
  }
...
## Loop over discretization level ltemp
for(ltemp in 1:8){
T = 100
L = ltemp
R=200
estMLPF = vector(mode = "numeric",length = R)
## Loop over 200 replications
for(h in 1:R){
...
for (p in 1:T) {
  ...
  for (j in 1:(2^l)) {
      dw[j] = rnorm(1, mean=0, sd = sqrt(0.5^l))
      fdis[j+1] = 0.02*fdis[j]*0.5^l + 0.2*fdis[j]*dw[j] + fdis[j]
  }
  ...
  cdis[1] = Xunc1[p,i]
  for(j in 1:(2^(l-1))){
    cdis[j+1] = 0.02*cdis[j]*0.5^(l-1) + 0.2*cdis[j]*(dw[2*j]+dw[2*j-1]) + cdis[j]
  }
...
for (p in 1:T) {
  ...
  fdis[2] = 0.02*fdis[1] + 0.2*fdis[1]*dw + fdis[1]
  Xun[p+1,i]=fdis[2]
  ## G Function
